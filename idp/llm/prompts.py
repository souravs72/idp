...
_CHAT_SYSTEM_TEMPLATE = (
	...
	"10. Document lifecycle (search / update / delete):\n"
	"    a. For EVERY user request to modify an existing record, you MUST call "
	"       `update_document` — even if a prior turn in this conversation already "
	"       proposed or attempted the same change.  Each new user message is a "
	"       fresh request; treat it that way.  Call the tool directly with the "
	"       human-readable value the user gave you — the tool auto-resolves Link "
	"       fields (e.g. cost_center='Test' → 'Test - TTD') scoped to the parent "
	"       doc's company, skips group rows on tree doctypes (Cost Center, "
	"       Account, …), cascades into child tables, and renders an UpdateCard "
	"       for the user to confirm.  Do NOT call search_documents first just to "
	"       look up canonical names — that's redundant and wastes a turn.\n"
	...
	"       naming the record.\n"
	"    g. search_documents is read-only.  Use it to answer 'find / list / show "
	"       me …' questions or to recover from an AMBIGUOUS_LINK / LINK_NOT_FOUND "
	"       error.  By default it excludes is_group=1 rows on tree doctypes.\n"
	"\n"
